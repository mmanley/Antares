/*
 * Copyright 2001-2006, Antares Inc. All Rights Reserved.
...
public:
	...
	BResources(const char *path, bool clobber = false);			// Antares only
	BResources(const entry_ref *ref, bool clobber = false);		// Antares only

	virtual ~BResources();

	status_t SetTo(const BFile *file, bool clobber = false);
	status_t SetTo(const char *path, bool clobber = false);		// Antares only
	status_t SetTo(const entry_ref *ref, bool clobber = false);	// Antares only
