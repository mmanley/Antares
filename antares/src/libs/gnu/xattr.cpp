...
static const char* const kXattrNamespace = "user.antares.";
...
struct AttributeName {
	...
	void Init(const char* antaresName, uint32 type)
	{
		if (type == B_XATTR_TYPE) {
			// a simple xattr -- copy the name verbatim
			strlcpy(name, antaresName, sizeof(name));
			this->type = type;
		} else {
			// a Antares attribute -- map the name
	...
	void Init(const char* xattrName)
	{
		if (strncmp(xattrName, kXattrNamespace, kXattrNamespaceLength) == 0) {
			// a Antares attribute -- extract the actual name and type
