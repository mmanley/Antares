...
/*!	Driver functions that adapt the FreeBSD driver to Antares's driver API.
	The actual driver functions are exported by the ANTARES_FBSD_DRIVER_GLUE
...
//	#pragma mark - Antares Driver API
...
_fbsd_init_driver(driver_t *driver)
{
	...
	if (ANTARES_DRIVER_REQUIRES(FBSD_TASKQUEUES)) {
