...
MainWindow::_AddDefaultButtons()
{
	...
	button = new LaunchButton("launch button", fLastID++, NULL,
		new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Antares-StyledEdit", true);

	// ShowImage
	button = new LaunchButton("launch button", fLastID++, NULL,
		new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Antares-ShowImage", true);

	// MediaPlayer
	button = new LaunchButton("launch button", fLastID++, NULL,
		new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Antares-MediaPlayer", true);

	// DeskCalc
	button = new LaunchButton("launch button", fLastID++, NULL,
		new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Antares-DeskCalc", true);

	// Terminal
	button = new LaunchButton("launch button", fLastID++, NULL,
		new BMessage(MSG_LAUNCH));
	fPadView->AddButton(button);
	button->SetTo("application/x-vnd.Antares-Terminal", true);
}
