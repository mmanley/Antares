...
static const char* kAppSig = "application/x-vnd.antares-icon_o_matic";
...
IconEditorApp::_InstallDocumentMimeType()
{
	...
	// set descriptions
	ret = mime.SetShortDescription("Antares Icon");
	if (ret < B_OK)
		fprintf(stderr, "Could not set short description of mime type: %s\n",
			strerror(ret));
	ret = mime.SetLongDescription("Native Antares vector icon");
	if (ret < B_OK)
