...
static const rgb_color kAntaresGreen = { 42, 131, 36, 255 };
static const rgb_color kAntaresOrange = { 255, 69, 0, 255 };
static const rgb_color kAntaresYellow = { 255, 176, 0, 255 };
...
AboutApp::AboutApp()
	: BApplication("application/x-vnd.Antares-About")
{
...
{
	fLogo = BTranslationUtils::GetBitmap(B_PNG_FORMAT, "antareslogo.png");
	...
	fScrollRunner(NULL)
{
	...
	if (uname(&unameInfo) == 0) {
		...
		if (sscanf(unameInfo.version, "r%ld", &revision) == 1) {
			...
			strcpy(string, "Revision ");
			strlcat(string, version, sizeof(string));
		} else {
			strcpy(string, "Unrevisioned");
		}
	}

	BStringView* revisionView = new BStringView("revisiontext", string);
	revisionView->SetExplicitAlignment(BAlignment(B_ALIGN_LEFT,
		B_ALIGN_VERTICAL_UNSET));
	...
	const StringVector& licenses, const StringVector& sources, const char* url)
{
	...
	fCreditsView->SetFontAndColor(&font, B_FONT_ALL, &kAntaresYellow);
...
void
AboutView::PickRandomAntares()
{
	BFile fortunes(
#ifdef __ANTARES__
		"/etc/fortunes/Antares",
#else
		"data/etc/fortunes/Antares",
#endif
		B_READ_ONLY);
	struct stat st;
	...
	free(buff);
	if (antaresList.CountItems() < 1)
		return;
	BString* s = (BString*)antaresList.ItemAt(rand() % antaresList.CountItems());
	BFont font(be_bold_font);
	font.SetSize(be_bold_font->Size());
	font.SetFace(B_BOLD_FACE | B_ITALIC_FACE);
	...
	fCreditsView->Insert("\n");
	while ((s = (BString*)antaresList.RemoveItem((int32)0))) {
		delete s;
	}
}
...
AboutView::_CreateCreditsView()
{
	...
	fCreditsView->SetFontAndColor(&font, B_FONT_ALL, &kAntaresGreen);
	fCreditsView->Insert("Antares \"Melissa\"\n");
	...
	snprintf(string, sizeof(string),
		COPYRIGHT_STRING "2009-%ld Alustead Systems a Division of NasuTek-Alliant Enterprises. ", year);

	fCreditsView->SetFontAndColor(be_plain_font, B_FONT_ALL, &kDarkGrey);
	fCreditsView->Insert(string);

	fCreditsView->SetFontAndColor(be_plain_font, B_FONT_ALL, &kDarkGrey);
	fCreditsView->Insert("The copyright to the original code of Antares is property of "
		"Haiku, Inc. or of the respective authors where expressly noted "
		"in the source. Any additional new code of the Antares project is property of "
		"Bretagne Systems or of the respective authors where expressly noted in the source"
		"\n"
		COPYRIGHT_STRING "2001-2010 The Haiku project."
		"\n\n");

	fCreditsView->SetFontAndColor(be_plain_font, B_FONT_ALL, &kLinkBlue);
	fCreditsView->InsertHyperText("http://alustead.nasutek.com/wiki/Antares_%22Melissa%22",
		new URLAction("http://alustead.nasutek.com/wiki/Antares_%22Melissa%22"));
	fCreditsView->Insert("\n\n");

	font.SetSize(be_bold_font->Size());
	font.SetFace(B_BOLD_FACE | B_ITALIC_FACE);

	fCreditsView->SetFontAndColor(&font, B_FONT_ALL, &kAntaresOrange);
	fCreditsView->Insert("Current Antares maintainers:\n");
	...
	fCreditsView->SetFontAndColor(&font, B_FONT_ALL, &kAntaresOrange);
	fCreditsView->Insert("Dedication:\n");

	fCreditsView->SetFontAndColor(be_plain_font, B_FONT_ALL, &kDarkGrey);
	fCreditsView->Insert(
		"The following is who I dedicate this release of Antares \"Melissa\" to. These "
		"people were there threw most of the development and deserve to have reconition "
		"in the credits.\n"
		"\n");
	...
	fCreditsView->Insert(
		...
		"Dan (GrimReaper)\n"
		"Dana (star)\n"
		"and Melissa, for whom this release was codenamed, for just being there for me as a good friend :)\n"
		"\n");

	fCreditsView->SetFontAndColor(be_plain_font, B_FONT_ALL, &kDarkGrey);
	fCreditsView->Insert("The following information below is the credit of the people who made the "
		"Haiku project. Without there contributations Haiku wouldn't be at the level it is today "
		"and because Antares Code is based on the hard work of all these people, its at the proper "
		"utmost respect to keep there credit lines intact and remain in the Antares code :)."
		"\n\n");
	...
	BPath apachePath;
	_GetLicensePath("Apache v2", apachePath);
	...
	// Antares license
	fCreditsView->Insert("The code that is unique to Antares, especially the "
		"kernel and all code that applications may link against, is "
		"distributed under the terms of the ");
	fCreditsView->InsertHyperText("Apache License Version 2.0",
		new OpenFileAction(apachePath.Path()));
	...
	_AddPackageCredit(PackageCredit("Haiku")
		.SetCopyright(COPYRIGHT_STRING
			"2001-2010 "
			"The Haiku project.")
		.SetLicense("MIT")
		.SetURL("http://www.haiku-os.org/"));
...
UptimeToString(char string[], size_t size)
{
	...
	if (seconds || str == string) {
		// Antares would be well-known to boot very fast.
