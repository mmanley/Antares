...
#if defined(ANTARES_HOST_PLATFORM_FREEBSD)
...
#if defined(ANTARES_HOST_PLATFORM_FREEBSD)
#	define antares_host_platform_read		antares_freebsd_read
#	define antares_host_platform_write	antares_freebsd_write
#	define antares_host_platform_readv	antares_freebsd_readv
#	define antares_host_platform_writev	antares_freebsd_writev
#else
#	define antares_host_platform_read		read
#	define antares_host_platform_write	write
#	define antares_host_platform_readv	readv
#	define antares_host_platform_writev	writev
#endif
...
_kern_read(int fd, off_t pos, void *buffer, size_t bufferSize)
{
	...
	// read
	ssize_t bytesRead = antares_host_platform_read(descriptor->fd, buffer,
		bufferSize);
	if (bytesRead < 0)
		return errno;
...
_kern_write(int fd, off_t pos, const void *buffer, size_t bufferSize)
{
	...
	// read
	ssize_t bytesWritten = antares_host_platform_write(descriptor->fd, buffer,
		bufferSize);
	if (bytesWritten < 0)
		return errno;
...
read_pos(int fd, off_t pos, void *buffer, size_t bufferSize)
{
	...
	// read
	ssize_t bytesRead = antares_host_platform_read(fd, buffer, bufferSize);
	if (bytesRead < 0) {
		errno = bytesRead;
		return -1;
	}
...
write_pos(int fd, off_t pos, const void *buffer, size_t bufferSize)
{
	...
	// read
	ssize_t bytesWritten = antares_host_platform_write(fd, buffer, bufferSize);
	if (bytesWritten < 0) {
		errno = bytesWritten;
		return -1;
	}
...
readv_pos(int fd, off_t pos, const struct iovec *vec, size_t count)
{
	...
	// read
	ssize_t bytesRead = antares_host_platform_readv(fd, vec, count);
	if (bytesRead < 0) {
		errno = bytesRead;
		return -1;
	}
...
writev_pos(int fd, off_t pos, const struct iovec *vec, size_t count)
{
	...
	// read
	ssize_t bytesWritten = antares_host_platform_writev(fd, vec, count);
	if (bytesWritten < 0) {
		errno = bytesWritten;
		return -1;
	}
