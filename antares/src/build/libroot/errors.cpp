...
#define BUILDING_ANTARES_ERROR_MAPPER 1
...
static map<int, int> sToAntaresErrorMap;
...
init_error_map()
{
	...
	#define ADD_ERROR(error) \
		sToAntaresErrorMap[error] = ANTARES_##error; \
		sToHostErrorMap[ANTARES_##error] = error;
...
// to_antares_error
static int
to_antares_error(int error)
{
	init_error_map();

	map<int, int>::iterator it = sToAntaresErrorMap.find(error);
	if (it != sToAntaresErrorMap.end())
		return it->second;

	return (error > 0 ? -error : error);
}

// _antares_build_strerror
char *
_antares_build_strerror(int errnum)
{
	return strerror(to_host_error(errnum));
}

// _antares_build_errno
int *
_antares_build_errno()
{
	...
	if (currentErrno == previousErrno) {
		...
		previousLocalErrno = localErrno = to_antares_error(errno);
	}

	return &localErrno;
}

// _antares_to_host_error
int
_antares_to_host_error(int error)
{
