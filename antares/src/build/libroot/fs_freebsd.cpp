...
ssize_t
antares_freebsd_read(int fd, void *buf, size_t nbytes)
{
	struct stat st;
	if (fstat(fd, &st) != 0)
		return -1;
...
ssize_t
antares_freebsd_write(int fd, const void *buf, size_t nbytes)
{
	struct stat st;
	if (fstat(fd, &st) != 0)
		return -1;
...
ssize_t
antares_freebsd_readv(int fd, const iovec *vecs, size_t count)
{
	ssize_t bytesRead = 0;

	for (size_t i = 0; i < count; i++) {
		ssize_t currentRead = antares_freebsd_read(fd, vecs[i].iov_base,
			vecs[i].iov_len);
...
ssize_t
antares_freebsd_writev(int fd, const struct iovec *vecs, size_t count)
{
	ssize_t bytesWritten = 0;

	for (size_t i = 0; i < count; i++) {
		ssize_t written = antares_freebsd_write(fd, vecs[i].iov_base,
			vecs[i].iov_len);
