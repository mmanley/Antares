...
{	
	switch(message->what) 
	{
		case BTN_CONNECT:	
		{
   	    	...
   	    	//debug
			BAlert *errormsg = new BAlert("errormsg", "The hidden wispy bush\nOver the green flower\nThe sea and connection have stopped.", "   Antares Error ;)   " , NULL, NULL, B_WIDTH_FROM_WIDEST, B_IDEA_ALERT);
		...
		case BTN_DISCONNECT:
		{
   	    	...
   	    	BAlert *errormsg = new BAlert("errormsg", "A late long rain\nOver an icy meadow\nBroken connection and dreams.", "   Antares Error ;)   ", NULL , NULL, B_WIDTH_FROM_WIDEST, B_IDEA_ALERT);   	   		
   	    	errormsg->SetShortcut(0, B_ESCAPE);
   	    	errormsg->Go();
	   	}    
	   		break;
