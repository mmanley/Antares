...
// TODO: this is an incompatibility between R5 and Antares and should be fixed!
#ifdef ANTARES_TARGET_PLATFORM_ANTARES
		rect.left++;
		rect.top++;
#else
...
#endif // ANTARES_TARGET_PLATFORM_ANTARES
			if (data == NULL) {
...
{
	// this works only in Antares! (the icon editor is built-in in R5's FileType)
#ifdef ANTARES_TARGET_PLATFORM_ANTARES
	BMessage message;
	...
	be_roster->Launch("application/x-vnd.antares-icon_o_matic", &message);
#endif
}
