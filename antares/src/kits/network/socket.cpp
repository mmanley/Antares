/*
 * Copyright 2002-2008, Antares, Inc. All Rights Reserved.
...
{
	struct sockaddr antaresAddr;

	if (check_r5_compatibility()) {
		convert_from_r5_sockaddr(&antaresAddr, address);
		address = &antaresAddr;
...
accept(int socket, struct sockaddr *_address, socklen_t *_addressLength)
{
	...
	if (r5compatible && _address != NULL) {
		address = &antaresAddr;
		addressLength = sizeof(antaresAddr);
	} else {
		address = _address;
	...
	if (r5compatible && _address != NULL) {
		convert_to_r5_sockaddr(_address, &antaresAddr);
