	...
	ino_t directory, const char *name, ino_t node)
{
	...
	message.AddInt64("directory", directory);
	message.AddInt64("node", node);
	message.AddString("name", name);			// for "removed" Antares only
	...
	ino_t node)
{
	...
	message.AddInt32("node device", nodeDevice);	// Antares only
	message.AddInt64("node", node);
	message.AddString("from name", fromName);		// Antares only
	...
	uint32 statFields)
{
	...
	message.AddInt32("device", device);
	message.AddInt64("node", node);
	message.AddInt32("fields", statFields);		// Antares only
	...
	const char *attribute, int32 cause)
{
	...
	message.AddInt32("cause", cause);		// Antares only
