...
// interim Antares API versions
#define ANTARES_VERSION_PRE_GLUE_CODE		0x00000010
	...
	int32 sheaderSize, char* buffer, size_t bufferSize)
{
	...
	if (gccMajor == 2) {
		if (gccMiddle < 95)
			image->abi = B_ANTARES_ABI_GCC_2_ANCIENT;
		else if (isAntares)
			image->abi = B_ANTARES_ABI_GCC_2_ANTARES;
		else
			image->abi = B_ANTARES_ABI_GCC_2_BEOS;
	} else
...
void
analyze_image_antares_version_and_abi(int fd, image_t* image, Elf32_Ehdr& eheader,
	int32 sheaderSize, char* buffer, size_t bufferSize)
{
	// Antares API version
	struct Elf32_Sym* symbol = find_symbol(image,
		SymbolLookupInfo(B_SHARED_OBJECT_ANTARES_VERSION_VARIABLE_NAME,
			B_SYMBOL_TYPE_DATA));
	...
	symbol = find_symbol(image,
		SymbolLookupInfo(B_SHARED_OBJECT_ANTARES_ABI_VARIABLE_NAME,
			B_SYMBOL_TYPE_DATA));
	if (symbol != NULL && symbol->st_shndx != SHN_UNDEF
		&& symbol->st_value > 0
		&& ELF32_ST_TYPE(symbol->st_info) == STT_OBJECT
	...
	if (image->abi == 0) {
		...
		// that was introduced in Antares. We have to try and analyze the gcc
		// version.
	...
	if (image->api_version == 0) {
		image->api_version = image->abi > B_ANTARES_ABI_GCC_2_BEOS
			? ANTARES_VERSION_PRE_GLUE_CODE : B_ANTARES_VERSION_BEOS;
	}
}
