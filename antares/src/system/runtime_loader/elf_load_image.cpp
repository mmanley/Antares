	...
	image_t** _image)
{
	...
	analyze_image_antares_version_and_abi(fd, image, eheader, sheaderSize,
	...
	if (type == B_APP_IMAGE) {
		#if __GNUC__ == 2
			if ((image->abi & B_ANTARES_ABI_MAJOR) == B_ANTARES_ABI_GCC_4)
				sSearchPathSubDir = "gcc4";
		#elif __GNUC__ == 4
			if ((image->abi & B_ANTARES_ABI_MAJOR) == B_ANTARES_ABI_GCC_2)
	...
	if (image->abi == B_ANTARES_ABI_GCC_2_ANCIENT)
