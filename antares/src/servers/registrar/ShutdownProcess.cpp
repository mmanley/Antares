...
public:
	...
	void SetWaitForAbortedOK()
	{
		...
		// TODO: Temporary work-around for a Antares bug.
...
ShutdownProcess::_NegativeQuitRequestReply(thread_id thread)
{
	...
	// Note: team ID == team main thread ID under Antares. When testing under R5
...
ShutdownProcess::_QuitNonApps()
{
	...
	while (get_next_team_info(&cookie, &teamInfo) == B_OK) {
		if (fVitalSystemApps.find(teamInfo.team) == fVitalSystemApps.end()) {
			...
			#ifdef __ANTARES__
				// Note: team ID == team main thread ID under Antares
