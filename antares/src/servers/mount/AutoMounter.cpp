...
AutoMounter::GetSupportedSuites(BMessage* data)
{
	...
	status_t status = data->AddString("suites",
		"suite/vnd.Antares-mount_server");
	if (status != B_OK)
		return status;
...
suggest_mount_flags(const BPartition* partition, uint32* _flags)
{
	...
	if (askReadOnly) {
		// Suggest to the user to mount read-only until Antares is more mature.
		BString string;
		...
		// TODO: Use distro name instead of "Antares"...
		if (!isBFS) {
			string << "The file system on this volume is not the Antares file "
				"system. It is strongly suggested to mount it in read-only "
				"mode. ";
		} else {
			string << "It is suggested to mount all additional Antares volumes "
				"in read-only mode. ";
		}
