...
ServerManager::Init()
{
	...
	fServerInfos = new(nothrow) ServerInfoMap();
	if (!fServerInfos)
...
ServerManager::AddServer(const NetAddress& address)
{
	...
	ExtendedServerInfo* serverInfo
		= new(nothrow) ExtendedServerInfo(address);
	if (!serverInfo)
		return B_NO_MEMORY;
...
//					"BROADCAST_MESSAGE_CLIENT_HELLO. Ignoring.\n"));
		...
		// create a new server info and add it
		ExtendedServerInfo* serverInfo
			= new(nothrow) ExtendedServerInfo(netAddress);
		if (!serverInfo)
			return B_NO_MEMORY;
		serverInfo->SetState(STATE_ADDING);
		Reference<ExtendedServerInfo> serverInfoReference(serverInfo, true);
		...
		ServerInfoTask* task = new(nothrow) ServerInfoTask(this, oldServerInfo,
			serverInfo);
		if (!task) {
			if (oldServerInfo) {
