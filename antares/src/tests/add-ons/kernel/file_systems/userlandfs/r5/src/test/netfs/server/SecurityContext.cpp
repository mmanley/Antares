...
User::Instantiate(BMessage* archive)
{
	if (!validate_instantiation(archive, "User"))
		return NULL;
	return new(nothrow) User(archive);
}
...
Share::Instantiate(BMessage* archive)
{
	if (!validate_instantiation(archive, "Share"))
		return NULL;
	return new(nothrow) Share(archive);
}
...
// UserMap
struct SecurityContext::UserMap : HashMap<String, User*> {
};

// ShareMap
struct SecurityContext::ShareMap : HashMap<String, Share*> {
};
...
// NodePathMap
struct SecurityContext::NodePathMap : HashMap<NodeRef, String> {
};

// PathNodeMap
struct SecurityContext::PathNodeMap : HashMap<String, NodeRef> {
};

// constructor
SecurityContext::SecurityContext()
	: BArchivable(),
	  Locker("security context"),
	  fUsers(new(nothrow) UserMap),
	  fShares(new(nothrow) ShareMap),
	  fPermissions(new(nothrow) PermissionMap),
	  fNode2Path(new(nothrow) NodePathMap),
	  fPath2Node(new(nothrow) PathNodeMap)
{
}
...
SecurityContext::Archive(BMessage* archive, bool deep) const
{
	...
	BMessage* tmpUserArchives = new(nothrow) BMessage[userCount];
	if (!tmpUserArchives)
		return B_NO_MEMORY;
...
SecurityContext::Instantiate(BMessage* archive)
{
	if (!validate_instantiation(archive, "SecurityContext"))
		return NULL;
	return new(nothrow) SecurityContext(archive);
}
...
// _AddNodePath
status_t
SecurityContext::_AddNodePath(const node_ref& ref, String* _path)
{
