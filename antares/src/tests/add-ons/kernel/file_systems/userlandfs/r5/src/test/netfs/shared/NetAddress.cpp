...
#if defined(ANTARES_TARGET_PLATFORM_DANO) || defined(ANTARES_TARGET_PLATFORM_DANO)
#	include <net/route.h>
...
// GetString
status_t
NetAddress::GetString(String* string, bool includePort) const
{
	if (!string)
		return B_BAD_VALUE;
	char buffer[32];
...
// Resolver
class NetAddressResolver::Resolver : public Referencable {
public:
	Resolver()
		: Referencable(false),
		  fLock()
...
NetAddressResolver::NetAddressResolver()
{
	...
	if (sResolver) {
		...
		sResolver = new(nothrow) Resolver;
		if (sResolver) {
