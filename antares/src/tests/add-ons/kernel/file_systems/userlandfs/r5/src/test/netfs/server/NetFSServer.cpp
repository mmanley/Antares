...
#ifdef ANTARES_TARGET_PLATFORM_BEOS
#	include <socket.h>
#else
#	include <unistd.h>
...
private:
	...
	int32 _Thread()
	{
		...
		if (error == B_OK) {
			clientConnection = new(nothrow) ClientConnection(fConnection,
...
public:
	ServerInfoSender(int socket, const ServerInfo& serverInfo)
		: Task("server info sender"),
		  fChannel(new(nothrow) InsecureChannel(socket)),
...
NetFSServer::_LoadSecurityContext(SecurityContext** _securityContext)
{
	...
	SecurityContext* securityContext = new(nothrow) SecurityContext;
	if (!securityContext)
		return B_NO_MEMORY;
...
NetFSServer::_InitSettings()
{
	...
	if (error != B_OK) {
		...
		if (error != B_OK) {
			...
			// use defaults
			// create a security context
			fSecurityContext = new(nothrow) SecurityContext;
			if (!fSecurityContext)
				return B_NO_MEMORY;
...
NetFSServer::_LoadSettings()
{
	...
	SecurityContext* securityContext
		= new(nothrow) SecurityContext(&securityContextArchive);
	if (!securityContext)
	...
	do {
		...
		if (error == B_OK) {
			ConnectionInitializer* initializer
				= new(nothrow) ConnectionInitializer(this, fConnectionListener,
					connection);
			if (initializer) {
...
NetFSServer::_ServerInfoConnectionListener()
{
	...
	while (!fTerminating) {
		...
		ServerInfoSender* sender = new(nothrow) ServerInfoSender(fd, info);
