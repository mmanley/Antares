/*
 * Copyright 2007, Antares. All rights reserved.
...
static void testDrawString(BView *view, BRect frame)
{
	...
	view->DrawString("Antares [ÖÜÄöüä]", BPoint(frame.left, baseline));
}
...
static void testDrawStringWithLength(BView *view, BRect frame)
{
	...
	view->SetHighColor(kBlack);
	view->DrawString("Antares [ÖÜÄöüä]", 13, BPoint(frame.left, baseline));
}
