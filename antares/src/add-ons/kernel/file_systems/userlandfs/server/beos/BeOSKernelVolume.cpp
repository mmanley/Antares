...
BeOSKernelVolume::ReadFSInfo(fs_info* info)
{
	...
	// Antares's fs_info equals BeOS's version
...
BeOSKernelVolume::ReadStat(void* node, struct stat* st)
{
	...
	// Antares's struct stat has an additional st_type field (for an attribute
	...
	size_t bufferSize, uint32 count, uint32* countRead)
{
	...
	*countRead = count;
	...
	// Antares's struct dirent equals BeOS's version
