...
new_vnode(fs_volume *_volume, ino_t vnodeID, void *privateNode,
	fs_vnode_ops *ops)
{
	AntaresKernelVolume* volume = AntaresKernelVolume::GetVolume(_volume);
...
get_vnode(fs_volume *_volume, ino_t vnodeID, void **privateNode)
{
	...
	*privateNode = ((AntaresKernelNode*)foundNode)->private_node;
...
{
	return AntaresKernelNode::GetNode(vnode)->GetVolume()->GetFSVolume();
}
...
{
	AntaresKernelIORequest* request = (AntaresKernelIORequest*)_request;
	...
	AntaresKernelIterativeFDIOCookie* cookie
		= new(std::nothrow) AntaresKernelIterativeFDIOCookie(fd, request, getVecs,
			finished, _cookie);
	if (cookie == NULL) {
...
{
	return ((AntaresKernelIORequest*)request)->isWrite;
}
...
{
	return ((AntaresKernelIORequest*)request)->offset;
}
...
{
	return ((AntaresKernelIORequest*)request)->length;
}
...
{
	AntaresKernelNode* node = (AntaresKernelNode*)vnode;
...
{
	// get the volume
	AntaresKernelVolume* volume = dynamic_cast<AntaresKernelVolume*>(
		FileSystem::GetInstance()->VolumeWithID(mountID));
	if (volume == NULL)
		return B_BAD_VALUE;

	// get the node
	AntaresKernelNode* node = volume->NodeWithID(vnodeID);
	if (node == NULL)
		return B_BAD_VALUE;
