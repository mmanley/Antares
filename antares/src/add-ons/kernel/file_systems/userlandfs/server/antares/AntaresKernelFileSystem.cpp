...
struct AntaresKernelFileSystem::IORequestHashDefinition {
	typedef int32					KeyType;
	typedef	AntaresKernelIORequest	ValueType;

	size_t HashKey(int32 key) const
		{ return key; }
	size_t Hash(const AntaresKernelIORequest* value) const
		{ return value->id; }
	bool Compare(int32 key, const AntaresKernelIORequest* value) const
		{ return value->id == key; }
	AntaresKernelIORequest*& GetLink(AntaresKernelIORequest* value) const
			{ return value->hashLink; }
};


// IORequestTable
struct AntaresKernelFileSystem::IORequestTable
...
struct AntaresKernelFileSystem::NodeCapabilitiesHashDefinition {
	typedef fs_vnode_ops*					KeyType;
	typedef	AntaresKernelNode::Capabilities	ValueType;
...
struct AntaresKernelFileSystem::NodeCapabilitiesTable
	: public BOpenHashTable<NodeCapabilitiesHashDefinition> {
};


// constructor
AntaresKernelFileSystem::AntaresKernelFileSystem(const char* fsName,
...
// destructor
AntaresKernelFileSystem::~AntaresKernelFileSystem()
{
...
// CreateVolume
status_t
AntaresKernelFileSystem::CreateVolume(Volume** _volume, dev_t id)
{
	...
	AntaresKernelVolume* volume
		= new(std::nothrow) AntaresKernelVolume(this, id, fFSModule);
	if (!volume)
		return B_NO_MEMORY;
...
// DeleteVolume
status_t
AntaresKernelFileSystem::DeleteVolume(Volume* volume)
{
	if (!volume || !dynamic_cast<AntaresKernelVolume*>(volume))
		return B_BAD_VALUE;
	delete volume;
	return B_OK;
}
...
// AddIORequest
status_t
AntaresKernelFileSystem::AddIORequest(AntaresKernelIORequest* request)
{
...
// GetIORequest
AntaresKernelIORequest*
AntaresKernelFileSystem::GetIORequest(int32 requestID)
{
	AutoLocker<Locker> _(fLock);

	AntaresKernelIORequest* request = fIORequests->Lookup(requestID);
	if (request != NULL)
		request->refCount++;
...
// PutIORequest
void
AntaresKernelFileSystem::PutIORequest(AntaresKernelIORequest* request,
	int32 refCount)
{
...
AntaresKernelFileSystem::GetNodeCapabilities(fs_vnode_ops* ops)
{
	AutoLocker<Locker> locker(fLock);
	...
	// create a new object
	capabilities = new(std::nothrow) AntaresKernelNode::Capabilities(ops,
		nodeCapabilities);
	if (capabilities == NULL)
		return NULL;
...
void
AntaresKernelFileSystem::PutNodeCapabilities(
	AntaresKernelNode::Capabilities* capabilities)
{
	AutoLocker<Locker> locker(fLock);
...
// _InitCapabilities
void
AntaresKernelFileSystem::_InitCapabilities()
{
	fCapabilities.ClearAll();

	// FS interface type
	fClientFSType = CLIENT_FS_ANTARES_KERNEL;
...
/*static*/ void
AntaresKernelFileSystem::_InitNodeCapabilities(fs_vnode_ops* ops,
	...
	FileSystem** _fileSystem)
{
	...
	AntaresKernelFileSystem* fileSystem
		= new(std::nothrow) AntaresKernelFileSystem(fsName, module);
	if (!fileSystem)
		RETURN_ERROR(B_NO_MEMORY);
