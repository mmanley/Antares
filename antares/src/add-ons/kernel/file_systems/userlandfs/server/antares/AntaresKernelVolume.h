 ...
 */
#ifndef USERLAND_FS_ANTARES_KERNEL_VOLUME_H
#define USERLAND_FS_ANTARES_KERNEL_VOLUME_H
...
class AntaresKernelFileSystem;
class AntaresKernelNode;


class AntaresKernelVolume : public Volume {
public:
								AntaresKernelVolume(FileSystem* fileSystem,
									dev_t id,
									file_system_module_info* fsModule);
	virtual						~AntaresKernelVolume();

	static	AntaresKernelVolume*	GetVolume(const fs_volume* volume);
	...
	inline	fs_volume*			GetFSVolume()	{ return &fVolume; }
			...
			status_t			NewVNode(ino_t vnodeID, void* privateNode,
									fs_vnode_ops* ops, AntaresKernelNode** node);
			status_t			PublishVNode(ino_t vnodeID, void* privateNode,
									fs_vnode_ops* ops, int type, uint32 flags,
									AntaresKernelNode** node);
			void				UndoNewVNode(AntaresKernelNode* node);
			void				UndoPublishVNode(AntaresKernelNode* node);

			AntaresKernelNode*	NodeWithID(ino_t vnodeID) const;
...
private:
	struct FSVolume : fs_volume {
		AntaresKernelVolume*	antaresVolume;
	};
...
private:
	...
	inline	AntaresKernelFileSystem* _FileSystem() const;
...
/*static*/ inline AntaresKernelVolume*
AntaresKernelVolume::GetVolume(const fs_volume* volume)
{
	return static_cast<const FSVolume*>(volume)->antaresVolume;
}


}	// namespace UserlandFS

using UserlandFS::AntaresKernelVolume;

#endif	// USERLAND_FS_ANTARES_KERNEL_VOLUME_H
