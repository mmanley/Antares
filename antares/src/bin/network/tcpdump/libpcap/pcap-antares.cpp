/*
 * Copyright 2006-2008, Antares, Inc. All Rights Reserved.
...
static int
pcap_read_antares(pcap_t* handle, int maxPackets, pcap_handler callback,
	u_char* userdata)
{
...
static int
pcap_inject_antares(pcap_t *handle, const void *buffer, size_t size)
{
...
static int
pcap_setfilter_antares(pcap_t *handle, struct bpf_program *filter)
{
...
static int
pcap_stats_antares(pcap_t *handle, struct pcap_stat *stats)
{
	ifreq request;
	...
	int /*timeout*/, char *errorBuffer)
{
	...
	handle->read_op = pcap_read_antares;
	handle->setfilter_op = pcap_setfilter_antares;
	handle->inject_op = pcap_inject_antares;
	handle->stats_op = pcap_stats_antares;
