/* Native-dependent code for Antares i386.
...
/* At kAntaresI386RegOffset[REGNUM] you'll find the offset in `struct
   debug_cpu_state' where the GDB register REGNUM is stored. */
static int kAntaresI386RegOffset[] = {
	REG_OFFSET (eax),
	REG_OFFSET (ecx),
	REG_OFFSET (edx),
	REG_OFFSET (ebx),
...
void
antares_supply_registers(int reg, const debug_cpu_state *cpuState)
{
	if (reg == -1) {
		int i;
		for (i = 0; i < NUM_REGS; i++)
			antares_supply_registers(i, cpuState);
	} else if (reg < I386_ST0_REGNUM) {
		int offset = kAntaresI386RegOffset[reg];
...
void
antares_collect_registers(int reg, debug_cpu_state *cpuState)
{
	if (reg == -1) {
		int i;
		for (i = 0; i < NUM_REGS; i++)
			antares_collect_registers(i, cpuState);
	} else if (reg < I386_ST0_REGNUM) {
		int offset = kAntaresI386RegOffset[reg];
