/* Antares native-dependent code common to multiple platforms.
...
#include "gdbcore.h"
#include "gdbthread.h"
#include "antares-nat.h"
#include "inferior.h"
#include "observer.h"
#include "regcache.h"
#include "solib-antares.h"
#include "symfile.h"
#include "target.h"

//#define TRACE_ANTARES_NAT
#ifdef TRACE_ANTARES_NAT
	#define TRACE(x)	printf x
#else
	#define TRACE(x)	while (false) {}
#endif

static struct target_ops *sAntaresTarget = NULL;
...
static char *antares_pid_to_str (ptid_t ptid);


static int
target_to_antares_signal(enum target_signal targetSignal)
{
	int i;
	...
	for (i = 0; sSignalMap[i].target != -1 || sSignalMap[i].antares != -1; i++) {
		if (targetSignal == sSignalMap[i].target)
			return sSignalMap[i].antares;
	}
...
antares_to_target_signal(int antaresSignal)
{
	int i;
	...
	for (i = 0; sSignalMap[i].target != -1 || sSignalMap[i].antares != -1; i++) {
		if (antaresSignal == sSignalMap[i].antares)
...
antares_find_thread(team_debug_info *teamDebugInfo, thread_id threadID)
{
...
static thread_debug_info *
antares_add_thread(team_debug_info *teamDebugInfo, thread_id threadID)
{
	struct thread_info *gdbThreadInfo;
	thread_debug_info *threadDebugInfo;

	if (threadID == teamDebugInfo->nub_thread) {
		error("antares_thread_added(): Trying to add debug nub thread (%ld)\n",
			threadID);
	}

	// find the thread first
	threadDebugInfo = antares_find_thread(teamDebugInfo, threadID);
	if (threadDebugInfo)
	...
	if (!threadDebugInfo)
		error("antares_thread_added(): Out of memory!\n");
	...
	TRACE(("antares_add_thread(): team %ld thread %ld added: "
...
static void
antares_remove_thread(team_debug_info *teamDebugInfo, thread_id threadID)
{
	thread_debug_info **info;
	for (info = &teamDebugInfo->threads; *info; info = &(*info)->next) {
		if ((*info)->thread == threadID) {
...
static void
antares_init_thread_list(team_debug_info *teamDebugInfo)
{
	...
	while (get_next_thread_info(teamDebugInfo->team, &cookie, &threadInfo)
		...
		if (threadInfo.thread != teamDebugInfo->nub_thread)
			antares_add_thread(teamDebugInfo, threadInfo.thread);
	}
}


static void
antares_cleanup_thread_list(team_debug_info *teamDebugInfo)
{
...
antares_find_image_insert_location(team_debug_info *teamDebugInfo,
	image_id imageID)
{
...
static extended_image_info *
antares_find_image(team_debug_info *teamDebugInfo, image_id imageID)
{
	extended_image_info *image = *antares_find_image_insert_location(
...
static extended_image_info *
antares_add_image(team_debug_info *teamDebugInfo, image_info *imageInfo)
{
	extended_image_info **imageP = antares_find_image_insert_location(
	...
	if (!image)
		error("antares_add_image(): Out of memory!");
...
static void
antares_remove_image(team_debug_info *teamDebugInfo, image_id imageID)
{
	extended_image_info **imageP = antares_find_image_insert_location(
		teamDebugInfo, imageID);
	extended_image_info *image = *imageP;
...
static void
antares_init_image_list(team_debug_info *teamDebugInfo)
{
	int32 cookie = 0;
	image_info info;
	while (get_next_image_info(teamDebugInfo->team, &cookie, &info) == B_OK)
		antares_add_image(teamDebugInfo, &info);
}


static void
antares_cleanup_image_list(team_debug_info *teamDebugInfo)
{
...
antares_get_next_image_info(int lastID)
{
	extended_image_info *image = *antares_find_image_insert_location(
...
static debug_event *
antares_enqueue_debug_event(debug_event_list *list, int32 message,
	debug_debugger_message_data *data, int32 size)
{
	debug_event *event = XMALLOC(debug_event);

	if (!event)
		error("antares_enqueue_debug_event(): Out of memory!\n");
...
static debug_event *
antares_dequeue_next_debug_event(debug_event_list *list)
{
...
static debug_event *
antares_remove_debug_event(debug_event_list *list, debug_event *eventToRemove)
{
	debug_event **event;
	...
	error("antares_remove_debug_event(): event %p not found in list %p\n",
		eventToRemove, list);
...
static void
antares_clear_debug_event_list(debug_event_list *list)
{
	debug_event *event;

	while ((event = antares_dequeue_next_debug_event(list)))
		xfree(event);
}


static debug_event *
antares_find_next_debug_event(debug_event_list *list,
...
static void
antares_read_pending_debug_events(team_debug_info *teamDebugInfo,
...
//		TRACE(("antares_read_pending_debug_events(): reading from debugger port "
//			"(%sblocking)...\n", (block ? "" : "non-")));
...
static bool
antares_thread_event_predicate(void *_closure, debug_event *event)
{
...
static void
antares_cleanup_team_debug_info()
{
	...
	antares_cleanup_thread_list(&sTeamDebugInfo);
	antares_cleanup_image_list(&sTeamDebugInfo);
}


static status_t
antares_send_debugger_message(team_debug_info *teamDebugInfo, int32 messageCode,
...
static void
antares_init_child_debugging (thread_id threadID, bool debugThread)
{
	thread_info threadInfo;
	status_t result;
	port_id nubPort;
	...
	// set the team debug flags
	{
		...
		antares_send_debugger_message(&sTeamDebugInfo,
			B_DEBUG_MESSAGE_SET_TEAM_FLAGS, &message, sizeof(message), NULL, 0);
	}


	// the fun can start: push the target and init the rest
	push_target(sAntaresTarget);

	antares_init_thread_list(&sTeamDebugInfo);
	antares_init_image_list(&sTeamDebugInfo);
...
static void
antares_continue_thread(team_debug_info *teamDebugInfo, thread_id threadID,
	uint32 handleEvent, bool step)
{
	...
	err = antares_send_debugger_message(teamDebugInfo,
...
static status_t
antares_stop_thread(team_debug_info *teamDebugInfo, thread_id threadID)
{
	// check whether we know the thread
	status_t err;
	thread_event_closure threadEventClosure;

	thread_debug_info *threadInfo = antares_find_thread(teamDebugInfo, threadID);
	if (!threadInfo)
	...
	if (err != B_OK) {
		TRACE(("antares_stop_thread(): failed to debug thread %ld: %s\n",
	...
	threadEventClosure.event = antares_find_next_debug_event(
		&teamDebugInfo->events, antares_thread_event_predicate,
		&threadEventClosure);

	// read all events pending on the port
	antares_read_pending_debug_events(teamDebugInfo,
		(threadEventClosure.event == NULL), antares_thread_event_predicate,
		&threadEventClosure);
...
static status_t
antares_get_cpu_state(team_debug_info *teamDebugInfo,
	debug_nub_get_cpu_state_reply *reply)
{
	...
	err = antares_stop_thread(teamDebugInfo, threadID);
	if (err != B_OK) {
...
static void
antares_child_open (char *arg, int from_tty)
{
	error ("Use the \"run\" command to start a child process.");
}

static void
antares_child_close (int x)
{
...
static void
antares_child_attach (char *args, int from_tty)
{
	...
	TRACE(("antares_child_attach(`%s', %d)\n", args, from_tty));
	...
	antares_init_child_debugging(threadID, true);

	TRACE(("antares_child_attach() done\n"));
}


static void
antares_child_detach (char *args, int from_tty)
{
	int detached = 1;
	status_t result;

	TRACE(("antares_child_detach(`%s', %d)\n", args, from_tty));
	...
	unpush_target (sAntaresTarget);
}


static void
antares_resume_thread(team_debug_info *teamDebugInfo, thread_debug_info *thread,
	int step, enum target_signal sig)
{
	thread_id threadID = (thread ? thread->thread : -1);
	int pendingSignal = -1;
	int signalToSend = target_to_antares_signal(sig);
	uint32 handleEvent = B_THREAD_DEBUG_HANDLE_EVENT;

	if (!thread || !thread->stopped) {
		TRACE(("antares_resume_thread(): thread %ld not stopped\n",
			threadID));
		return;
	}

	if (thread->reprocess_event > 0) {
		TRACE(("antares_resume_thread(): thread %ld is expected to "
	...
	// resume thread
	antares_continue_thread(teamDebugInfo, threadID, handleEvent, step);
	thread->stopped = false;
}


static void
antares_child_resume (ptid_t ptid, int step, enum target_signal sig)
{
	team_id teamID = ptid_get_pid(ptid);
	thread_id threadID = ptid_get_tid(ptid);
	thread_debug_info *thread;

	TRACE(("antares_child_resume(`%s', step: %d, signal: %d)\n",
		antares_pid_to_str(ptid), step, sig));

	if (teamID < 0) {
		// resume all stopped threads (at least all antares_wait_child() has
		// reported to be stopped)
		for (thread = sTeamDebugInfo.threads; thread; thread = thread->next) {
			if (thread->stopped)
				antares_resume_thread(&sTeamDebugInfo, thread, step, sig);
		}
	} else {
		// resume the thread in question
		thread = antares_find_thread(&sTeamDebugInfo, threadID);
		if (thread) {
			antares_resume_thread(&sTeamDebugInfo, thread, step, sig);
		} else {
			printf_unfiltered ("antares_child_resume(): unknown thread %ld\n",
				threadID);
		}
	}
...
static ptid_t
antares_child_wait_internal (team_debug_info *teamDebugInfo, ptid_t ptid,
	struct target_waitstatus *ourstatus, bool *ignore, bool *selectThread)
{
	...
	*selectThread = false;
	...
	if (threadID >= 0
		&& (thread = antares_find_thread(teamDebugInfo, threadID)) != NULL
		&& thread->stopped) {
		...
		TRACE(("antares_child_wait_internal(): thread %ld already stopped. "
		...
		event = antares_remove_debug_event(&teamDebugInfo->events,
			threadEventClosure.event);

		if (!event) {
			TRACE(("antares_child_wait_internal(): got no event!\n"));
			return retval;
		}

		TRACE(("antares_child_wait_internal(): got event: %u, thread: %ld, "
			"team: %ld, nub port: %ld\n", event->message,
	...
	*ignore = false;
	...
	switch (event->message) {
		...
		case B_DEBUGGER_MESSAGE_SIGNAL_RECEIVED:
			...
			ourstatus->kind = TARGET_WAITKIND_STOPPED;
			ourstatus->value.sig = antares_to_target_signal(pendingSignal);
			break;
		...
		case B_DEBUGGER_MESSAGE_THREAD_CREATED:
		{
			...
			if (newThread != teamDebugInfo->nub_thread)
				antares_add_thread(teamDebugInfo, newThread);
			*ignore = true;
			break;
		}
		...
		case B_DEBUGGER_MESSAGE_THREAD_DELETED:
			...
			antares_remove_thread(teamDebugInfo,
...
//				antares_add_image(teamDebugInfo,
//					&event->data.image_created.info);
antares_cleanup_image_list(teamDebugInfo);
antares_init_image_list(teamDebugInfo);
...
TRACE(("antares_child_wait_internal(): ignoring exec (%d)\n",
inferior_ignoring_startup_exec_events));
				...
				if (reprocessEvent < 0) {
TRACE(("antares_child_wait_internal(): B_APP_IMAGE created, reprocess < 0\n"));
					// the first time the event is processed: send the
					// `loaded' event
					reprocessEvent = 2;
				} else if (reprocessEvent == 2) {
TRACE(("antares_child_wait_internal(): B_APP_IMAGE created, reprocess -> exec\n"));
					// the second time the event is processed: send the `exec'
					// event
		...
		case B_DEBUGGER_MESSAGE_IMAGE_DELETED:
			antares_remove_image(teamDebugInfo,
		...
		{
TRACE(("antares_child_wait_internal(): B_DEBUGGER_MESSAGE_HANDED_OVER: causing "
"thread: %ld\n", event->data.handed_over.causing_thread));
	...
	if (!*ignore && event->data.origin.thread >= 0) {
		struct thread_debug_info *thread = antares_find_thread(teamDebugInfo,
			event->data.origin.thread);
		if (thread->last_event)
		...
		if (event->data.origin.thread >= 0) {
			antares_continue_thread(teamDebugInfo, event->data.origin.thread,
...
static ptid_t
antares_child_wait (ptid_t ptid, struct target_waitstatus *ourstatus)
{
	ptid_t retval;
	bool ignore = true;
	bool selectThread = false;

	TRACE(("antares_child_wait(`%s', %p)\n",
		antares_pid_to_str(ptid), ourstatus));

	do {
		retval = antares_child_wait_internal(&sTeamDebugInfo, ptid, ourstatus,
			&ignore, &selectThread);
		if (selectThread)
			ptid = retval;
	} while (ignore);

	TRACE(("antares_child_wait() done: `%s'\n", antares_pid_to_str(retval)));
...
static void
antares_child_fetch_inferior_registers (int reg)
{
	debug_nub_get_cpu_state_reply reply;

	TRACE(("antares_child_fetch_inferior_registers(%d)\n", reg));

	// get the CPU state
	antares_get_cpu_state(&sTeamDebugInfo, &reply);

// printf("antares_child_fetch_inferior_registers(): eip: %p, ebp: %p\n",
// (void*)reply.cpu_state.eip, (void*)reply.cpu_state.ebp);

	// supply the registers (architecture specific)
	antares_supply_registers(reg, &reply.cpu_state);
}


static void
antares_child_store_inferior_registers (int reg)
{
	status_t err;
	thread_id threadID = ptid_get_tid(inferior_ptid);
	debug_nub_get_cpu_state_reply reply;
	...
	TRACE(("antares_child_store_inferior_registers(%d)\n", reg));

	// get the current CPU state
	antares_get_cpu_state(&sTeamDebugInfo, &reply);

	// collect the registers (architecture specific)
	antares_collect_registers(reg, &reply.cpu_state);

	// set the new CPU state
	message.thread = threadID;
	memcpy(&message.cpu_state, &reply.cpu_state, sizeof(debug_cpu_state));
	err = antares_send_debugger_message(&sTeamDebugInfo,
...
static void
antares_child_prepare_to_store (void)
{
	// Since we always fetching the current state in
	// antares_child_store_inferior_registers(), this should be a no-op.
}

static int
antares_child_deprecated_xfer_memory (CORE_ADDR memaddr, char *myaddr, int len,
	int write, struct mem_attrib *attrib, struct target_ops *target)
{
	TRACE(("antares_child_deprecated_xfer_memory(0x%8lx, %p, %d, %d, %p, %p)\n",
...
{
TRACE(("antares_child_deprecated_xfer_memory() failed: %lx\n",
(err != B_OK ? err : reply.error)));
			return 0;
}

TRACE(("antares_child_deprecated_xfer_memory(): -> %ld\n", reply.size));
		return reply.size;
	} else {
		// read
...
LONGEST
antares_child_xfer_partial (struct target_ops *ops, enum target_object object,
	...
	LONGEST len)
{
	...
	switch (object) {
		...
		{
			int write = !readbuf;
			return antares_child_deprecated_xfer_memory (offset,
...
static void
antares_child_files_info (struct target_ops *ignore)
{
...
static void
antares_child_kill_inferior (void)
{
	status_t err;
	thread_id teamID = ptid_get_pid(inferior_ptid);

	TRACE(("antares_child_kill_inferior()\n"));
...
static void
antares_child_stop_inferior (void)
{
	status_t err;
	thread_id threadID = ptid_get_tid(inferior_ptid);

	TRACE(("antares_child_stop_inferior()\n"));
...
static void
antares_init_debug_create_inferior(int pid)
{
	extern int stop_after_trap;

	// fix inferior_ptid -- fork_inferior() sets the process ID only
	inferior_ptid = ptid_build (pid, 0, pid);
		// team ID == team main thread ID under Antares

	antares_init_child_debugging(pid, false);
...
// they may have.
	...
	while (true) {
		...
		if (stop_signal == TARGET_SIGNAL_TRAP) {
			thread = antares_find_thread(&sTeamDebugInfo, pid);
...
//
//		thread = antares_find_thread(&sTeamDebugInfo, pid);
//TRACE(("antares_init_debug_create_inferior(): wait_for_inferior() returned: "
...
//			&& thread->last_event->data.image_created.info.type
//				== B_APP_IMAGE) {
//TRACE(("antares_init_debug_create_inferior(): Got an `app image created' "
//"message\n"));
//			break;
//		}
//
...
static void
antares_child_create_inferior (char *exec_file, char *allargs, char **env,
	int from_tty)
{
	TRACE(("antares_child_create_inferior(`%s', `%s', %p, %d)\n", exec_file,
		allargs, env, from_tty));

	fork_inferior (exec_file, allargs, env, wait_for_debugger,
		antares_init_debug_create_inferior, NULL, NULL);
...
static void
antares_child_mourn_inferior (void)
{
	TRACE(("antares_child_mourn_inferior()\n"));
...
static int
antares_child_can_run (void)
{
	return 1;
}

static int
antares_child_thread_alive (ptid_t ptid)
{
	thread_info info;

	TRACE(("antares_child_thread_alive(`%s')\n", antares_pid_to_str(ptid)));

	return (get_thread_info(ptid_get_tid(ptid), &info) == B_OK);
}

static char *
antares_pid_to_str (ptid_t ptid)
{
...
char *
antares_child_pid_to_exec_file (int pid)
{
...
void
_initialize_antares_nat (void)
{
	// child operations
	struct target_ops *t = XZALLOC (struct target_ops);
	t->to_shortname = "child";
	t->to_longname = "Antares child process";
	t->to_doc = "Antares child process (started by the \"run\" command).";
	t->to_open = antares_child_open;
	t->to_close = antares_child_close;
	t->to_attach = antares_child_attach;
	t->to_detach = antares_child_detach;
	t->to_resume = antares_child_resume;
	t->to_wait = antares_child_wait;
	t->to_fetch_registers = antares_child_fetch_inferior_registers;
	t->to_store_registers = antares_child_store_inferior_registers;
	t->to_prepare_to_store = antares_child_prepare_to_store;
	t->deprecated_xfer_memory = antares_child_deprecated_xfer_memory;
	t->to_xfer_partial = antares_child_xfer_partial;
	t->to_files_info = antares_child_files_info;
	...
	t->to_kill = antares_child_kill_inferior;
	t->to_stop = antares_child_stop_inferior;
	t->to_create_inferior = antares_child_create_inferior;
	t->to_mourn_inferior = antares_child_mourn_inferior;
	t->to_can_run = antares_child_can_run;
	t->to_thread_alive = antares_child_thread_alive;
// How about to_find_new_threads? Perhaps not necessary, as we could be informed
// about thread creation/deletion.
	t->to_pid_to_str = antares_pid_to_str;
	t->to_pid_to_exec_file = antares_child_pid_to_exec_file;
