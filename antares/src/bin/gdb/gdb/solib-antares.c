/* Shared library support for Antares.
...
#include "defs.h"
#include "antares-nat.h"	// TODO: Needs to be removed. See there!
#include "inferior.h"
#include "objfiles.h"
...
//#define TRACE_SOLIB_ANTARES
#ifdef TRACE_SOLIB_ANTARES
...
static antares_image_info *
antares_get_app_image()
{
	antares_image_info *image;
	int lastID = -1;

	while ((image = antares_get_next_image_info(lastID)) != NULL) {
...
{
	antares_image_info *appImageInfo = antares_get_app_image();
...
static void
antares_free_so (struct so_list *so)
{
	xfree (so->lm_info);
}
...
static void
antares_clear_solib (void)
{
}


static void
antares_solib_create_inferior_hook (void)
{
...
static void
antares_special_symbol_handling (void)
{
}


static struct so_list *
antares_current_sos (void)
{
	int lastID = -1;
	antares_image_info *image;
...
static int
antares_open_symbol_file_object (void *from_ttyp)
{
	// Note: I have never seen this function being called. The Sun OS
	// implementation is a no-op.
	antares_image_info *appImage = antares_get_app_image();

	TRACE(("antares_open_symbol_file_object(%p)\n", from_ttyp));

	if (!appImage) {
		TRACE(("antares_open_symbol_file_object(): No app image!\n"));
		return 0;
	}
...
static int
antares_in_dynsym_resolve_code (CORE_ADDR pc)
{
	// No dynamic resolving implemented in Antares yet.
	return 0;
}
...
static struct target_so_ops antares_so_ops;

extern initialize_file_ftype _initialize_antares_solib;

void
_initialize_antares_solib (void)
{
	antares_so_ops.relocate_section_addresses = antares_relocate_section_addresses;
	antares_so_ops.free_so = antares_free_so;
	antares_so_ops.clear_solib = antares_clear_solib;
	antares_so_ops.solib_create_inferior_hook = antares_solib_create_inferior_hook;
	antares_so_ops.special_symbol_handling = antares_special_symbol_handling;
	antares_so_ops.current_sos = antares_current_sos;
	antares_so_ops.open_symbol_file_object = antares_open_symbol_file_object;
	antares_so_ops.in_dynsym_resolve_code = antares_in_dynsym_resolve_code;

	/* FIXME: Don't do this here.  *_gdbarch_init() should set so_ops. */
	current_target_so_ops = &antares_so_ops;
}
