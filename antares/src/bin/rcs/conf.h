...
#if defined(O_BINARY) && !(defined(__BEOS__) || defined(__ANTARES__))
...
#define DIFF3 "/bin/diff3" /* name of 'diff3' program */
...
#define ED "ed" /* name of 'ed' program (used only if !DIFF3_BIN) */
...
#if has_sys_siglist && !defined(sys_siglist) && !(defined(__BEOS__) || defined(__ANTARES__))
