...
This simple script traverses the antares sources and creates (incomplete) *.pro
files in order to make the antares sources available within the qt-creator IDE.
...
if (!@ARGV) {
	die "usage: $0 <antares-top-path>\n";
}

my $antaresTop = shift @ARGV;
if (!-e "$antaresTop/ReadMe.cross-compile") {
	die "'$antaresTop/ReadMe.cross-compile' not found - not a antares top!\n";
}
...
find({ wanted => \&process, no_chdir => 1}, 
	("$antaresTop/headers", "$antaresTop/src"));

writeProFile("$antaresTop/antares.pro", { subdirs => ['headers', 'src'] });
