...
/* definitions for Antares Terminal settings file */
/* from TermConst.h */
...
static const char *kAntaresTerminalAppSig = "application/x-vnd.Antares-Terminal";
...
status_t	ApplyThemeAntares(BMessage &theme, uint32 flags);
status_t	MakeThemeAntares(BMessage &theme, uint32 flags);
status_t	LoadAntaresTerminalSettings(BMessage &into);
status_t	SaveAntaresTerminalSettings(BMessage &from);
};
...
#endif
	// Some people use the Antares terminal in BeOS.
	err = ApplyThemeAntares(theme, flags);
	if (err)
		return err;
	return B_OK;
}
...
{
#ifdef __ANTARES__
	return MakeThemeAntares(theme, flags);
#else
...
status_t
TerminalThemesAddon::ApplyThemeAntares(BMessage &theme, uint32 flags)
{
	BMessage termpref;
	BMessage lines;
	status_t err;
	...
	if (flags & UI_THEME_SETTINGS_SAVE && AddonFlags() & Z_THEME_ADDON_DO_SAVE) {
		SaveAntaresTerminalSettings(lines);
	}
	...
	if (flags & UI_THEME_SETTINGS_APPLY && AddonFlags() & Z_THEME_ADDON_DO_APPLY) {
		...
		for (i = 0; i < count; i++) {
			if (be_roster->GetRunningAppInfo((team_id)(teamList.ItemAt(i)), &ainfo) == B_OK) {
				if (!strcmp(ainfo.signature, kAntaresTerminalAppSig)) {
					err = B_OK;
					//XXX: WRITEME
/*					BMessage msg(MSG_R5_SET_PREF);
					BMessenger msgr(NULL, ainfo.team);
...
status_t
TerminalThemesAddon::MakeThemeAntares(BMessage &theme, uint32 flags)
{
	BMessage termpref;
	BMessage lines;
	status_t err;
	...
	err = LoadAntaresTerminalSettings(lines);
...
status_t
TerminalThemesAddon::LoadAntaresTerminalSettings(BMessage &into)
{
	BPath pTermPref;
	char buffer[1024];
	char key[B_FIELD_NAME_LENGTH], data[512];
...
#else
	pTermPref.Append("AntaresTerminal_settings");
#endif
	// cf PrefHandler.cpp
...
status_t
TerminalThemesAddon::SaveAntaresTerminalSettings(BMessage &from)
{
	BMessage settings;
	status_t err;
	BPath pTermPref;
	...
	// load existing
	err = LoadAntaresTerminalSettings(settings);
