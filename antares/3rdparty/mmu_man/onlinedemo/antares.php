...
/*
 * antares.php - an online Antares demo using qemu and vnc.
 *
...
if (file_exists('antares.conf.php'))
	include('antares.conf.php');


// name of the page
define("PAGE_TITLE", "Antares Online Demo");
...
define("QEMU_IMAGE_PATH", "/home/revol/antares.image");
// BAD: let's one download the image
//define("QEMU_IMAGE_PATH", dirname($_SERVER['SCRIPT_FILENAME']) . "/antares.image");
...
define("QEMU_SESSFILE_TMPL", "qemu-antares-session-");
define("QEMU_PIDFILE_TMPL", "qemu-antares-pid-");
// name of session variable holding the qemu slot; not yet used correctly
define("QEMU_IDX_VAR", "QEMU_ANTARES_SESSION_VAR");
...
function out($str)
{
	echo "<div class=\"antares_online_out\">$str</div>\n";
	ob_flush();
	flush();
}
...
function dbg($str)
{
	echo "<div class=\"antares_online_debug\">$str</div>\n";
	ob_flush();
	flush();
}
...
function err($str)
{
	echo "<div class=\"antares_online_error\">$str</div>\n";
	ob_flush();
	flush();
}
...
function output_options_form()
{
	...
	echo "<table border=\"0\" class=\"antares_online_form\">\n";
	...
	echo "<tr ";
	if (count($modes) < 2)
		echo "class=\"antares_online_disabled\"";
	echo ">\n";
	echo "<td align=\"right\">\n";
...
function output_applet_code($external_only=false)
{
	...
	if ($external_only)
		return;
	echo "<a name=\"antares_online_applet\"></a>";
	echo "<center>";
	...
	echo "<script>\n";
	echo "<!--\n";
	echo "scrollToAnchor(\"antares_online_applet\");";
	echo "//--></script>\n";
	ob_flush();
	flush();
}
...
<link rel="shortcut icon" href="http://www.antares-os.org/themes/shijin/favicon.ico" type="image/x-icon" />
<style type="text/css">
<!--
 /* basic style */
...
a:hover { color:pink; }
.antares_online_form { color: white; }
.antares_online_disabled { color: grey; }
.antares_online_out { color: white; }
.antares_online_debug { color: orange; }
.antares_online_error { color: red; font-weight: bold; }
.antares_online_applet { background-color: <?php echo BGCOLOR; ?>; }
-->
</style>
